...
TREE_DUMP = TREE_FILE.replace('.trees', '.biallelic.recapitated.trees')
...
rts = pyslim.recapitate(ts, ancestral_Ne=25000, recombination_rate=1e-8, random_seed=1)
...
sample_nodes = rts.samples()   # actual sample node IDs, not a raw range
...
tmrcas, n_failed = [], 0
for tree in itertools.islice(rts.trees(), 0, None, 1000):
...
multiallelic_site_ids = np.array(
    [site.id for site in rts.sites() if len(site.mutations) > 1]
)
print(f"Removing {len(multiallelic_site_ids)} multiallelic sites "
      f"out of {rts.num_sites}", flush=True)
